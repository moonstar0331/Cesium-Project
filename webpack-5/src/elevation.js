...
// Elavation Coordinate Label Display Function && Set Reference Height
export function setReferenceHeight(viewer, event) {
  const cartesian = viewer.camera.pickEllipsoid(
    new Cartesian2(event.clientX, event.clientY),
    viewer.scene.globe.ellipsoid,
  );
  if (cartesian) {
    ...
    const coord = viewer.entities.getById("coordinate");
    const referHeight = document.getElementById("reference-height");
